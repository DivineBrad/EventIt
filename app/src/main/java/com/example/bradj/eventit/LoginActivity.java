...
public class LoginActivity extends AppCompatActivity implements LoaderCallbacks<Cursor>, View.OnClickListener {
    ...
    private void attemptLogin() {
        ...
        if (cancel) {
            ...
            showProgress(true);
            attemptLoginTask(email,password);
        }
    }
     ...
     * the user.
     */
    public void attemptLoginTask(String email, String password) {

                final User loginUser=new User();
                loginUser.setEmail(email);
                loginUser.setPass(password);
                mUserService.getLoginUser(loginUser).enqueue(new Callback<User>() {
                    @Override
                    public void onResponse(Call<User> call, Response<User> response) {
                        if(response.isSuccessful()) {
                            User user=response.body();
                            loginUtil.createUserSession(getApplicationContext(),(int)response.body().getUserId(), user);
